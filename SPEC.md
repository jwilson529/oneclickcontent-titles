...
## 11. Product Guidance: Discover and Top Stories Cards

When the user selects a Discover or Top Stories intent, the plugin must bias title generation toward the behaviors of Google Discover style cards. Implementations must reflect the following guidance inside the generation prompt or model instructions:

- Treat Discover cards as SERP objects driven by selection and attention, not classic keyword ranking.
- Favor entity-led headlines that start with a recognized person, brand, or organization, then add unresolved tension.
- Use engineered ambiguity to create curiosity without being misleading.
- Prioritize freshness, velocity, and engagement over evergreen completeness.
- Reinforce that image-led context matters, but keep titles independent from the image itself.
- Emphasize that authority and domain trust influence selection, not just content quality.

This guidance must be translated into generation instructions so the model produces titles that align with Discover card behavior while still meeting plugin constraints like length and keyword inclusion.
